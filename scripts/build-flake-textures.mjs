...
/*
  UNDER THE TOPCOAT THE FLOOR IS CALMER THAN LOOSE FLAKE IN A PHOTOGRAPHER'S
  LIGHTBOX. The sample is shot bright and contrasty to sell the colours; chips
  broadcast into resin and buried under clear sit a little darker and a little
  closer together in tone.

  Gentle on purpose — this is a slight settle, not a grade. White 255 -> 232,
  black 0 -> 8, so the four tones Dovetail is built from stay separate.
*/
const TOPCOAT_GAIN = 0.88
const TOPCOAT_LIFT = 8
...
async function build(slug, srcFile) {
  ...
  */
  ...
  /*
    The base coat, shown only if a texture ever fails to load. Derived from the
    sample rather than invented, and darker, because bare base coat between
    chips is what you would see through a thin spot.
  */
  const base = mean.map((v) => Math.round(v * 0.82))

  return { slug, chipPx, srcPx, mean, sMean, base }
}
...
for (const f of files) {
  ...
  */
  ...
  manifest[slug] = { base: `#${r.base.map((v) => v.toString(16).padStart(2, '0')).join('')}` }
  console.log(
 ...
 *  ${TILE}px tile ≈ ${(inchesPerTile / 12).toFixed(2)}ft of slab. */
